...
These results shows the amount of heterogeneity ("between-trial variance") is estimated to be $$\hat{\tau}^2 = 0$$ with a standard error of $$SE\hat{\tau}^2 = 0.0064$$ and the I<sup>2</sup> = 0, H<sup>2</sup> = 1. Since the I<sup>2</sup> = 0, the fixed-effect model can be a more appropriate method. And test for heterogeneity is Q = 4.5908637, P−value = 0.8684174,which shows less heterogeneity between thses studies.
